...
export const INTERMEDIATE_LESSONS: Lesson[] = [
  ...
  },
  {
    ...
    description: 'Master Angular Signals - the future of reactivity including linkedSignal.',
    level: 'intermediate', duration: '45 min',
    objectives: ['Create and use signals', 'Understand computed signals', 'Master effects', 'Use linkedSignal for derived state'],
    quiz: [
      {
        id: 1,
    ...
    topics: [
      ...
      { id: 'effects', title: 'Effects', content: '**Practice - Side Effects:**\n```typescript\n@Component({})\nexport class LoggerComponent {\n  count = signal(0);\n\n  constructor() {\n    effect(() => {\n      console.log(\'Count changed:\', this.count());\n      return () => console.log(\'Cleaning up\');\n    });\n  }\n}\n```\n\n**Key Takeaways:**\n1. Effects run when signals they read change\n2. Effects auto-track dependencies\n3. Cleanup functions run before re-execution' },
      { id: 'linked-signal', title: 'linkedSignal', content: '**linkedSignal - Writable Derived State:**\n\n`linkedSignal` creates a signal that can be both read AND written, derived from other signals.\n\n```typescript\n@Component({})\nexport class ItemComponent {\n  items = signal([\'Apple\', \'Banana\', \'Cherry\']);\n  selectedIndex = signal(0);\n  \n  // linkedSignal: derived but writable\n  selectedItem = linkedSignal(() => this.items()[this.selectedIndex()]);\n}\n```\n\n**Updating linkedSignal:**\n```typescript\n@Component({})\nexport class FormComponent {\n  form = signal({ name: \'\', email: \'\' });\n  \n  // linkedSignal: can be set externally\n  nameField = linkedSignal(() => this.form().name);\n  \n  updateName(newName: string) {\n    this.form.update(f => ({ ...f, name: newName }));\n    this.nameField.set(newName); // Set both\n  }\n}\n```\n\n**linkedSignal vs computed:**\n- `computed`: Read-only, derives value automatically\n- `linkedSignal`: Read-write, can be set externally\n- Use linkedSignal when you need derived state that can also be updated directly\n\n**Key Takeaways:**\n1. linkedSignal creates writable derived signals\n2. When source changes, linkedSignal updates\n3. Can also be set directly with .set() or .update()\n4. Use when computed\'s read-only limitation is too restrictive' }
    ]
  },
  {
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'signal-forms', title: 'Signal Forms (Angular 22)', content: '**Practice - Signal-Based Forms:**\n```typescript\nimport { form, field, required, email } from \'@angular/forms/signals\';\n\n@Component({})\nexport class UserFormComponent {\n  userForm = form({\n    name: field(\'\', { validators: [required()] }),\n    email: field(\'\', { validators: [required(), email()] })\n  });\n}\n```\n\n**Key Takeaways:**\n1. Signal Forms provide reactive form state with signals\n2. Use field() instead of FormControl\n3. Validation integrates with the signal reactivity model' },
      { id: 'form-arrays', title: 'FormArrays', content: '**FormArray - Dynamic Form Fields:**\n\nFormArray manages a dynamic list of FormControls (e.g., adding/removing skills).\n\n```typescript\nimport { FormArray, FormControl, FormGroup } from \'@angular/forms\';\n\n@Component({})\nexport class SkillsFormComponent {\n  form = new FormGroup({\n    name: new FormControl(\'\'),\n    skills: new FormArray([\n      new FormControl(\'Angular\'),\n      new FormControl(\'TypeScript\')\n    ])\n  });\n\n  get skills(): FormArray {\n    return this.form.get(\'skills\') as FormArray;\n  }\n\n  addSkill() {\n    this.skills.push(new FormControl(\'\'));\n  }\n\n  removeSkill(index: number) {\n    this.skills.removeAt(index);\n  }\n}\n```\n\n**Template:**\n```html\n<form [formGroup]="form">\n  <input formControlName="name" />\n  \n  <div formArrayName="skills">\n    @for (skill of skills.controls; track $index; let i = $index) {\n      <input [formControlName]="i" />\n      <button type="button" (click)="removeSkill(i)">Remove</button>\n    }\n  </div>\n  \n  <button type="button" (click)="addSkill()">Add Skill</button>\n</form>\n```\n\n**Key Takeaways:**\n1. FormArray manages dynamic lists of controls\n2. Use .push() and .removeAt() to modify\n3. Template uses formArrayName and index-based binding' },
      { id: 'async-validators', title: 'Async Validators', content: '**Async Validators - Server-Side Validation:**\n\nAsync validators return a Promise or Observable and run asynchronously.\n\n```typescript\nimport { AsyncValidatorFn } from \'@angular/forms\';\nimport { map, debounceTime, switchMap, first } from \'rxjs/operators\';\n\n// Async validator factory\nexport function uniqueEmailValidator(userService: UserService): AsyncValidatorFn {\n  return (control) => {\n    return control.valueChanges.pipe(\n      debounceTime(400),\n      switchMap(email => userService.checkEmail(email)),\n      map(taken => taken ? { emailTaken: true } : null),\n      first()\n    );\n  };\n}\n\n// Usage\n@Component({})\nexport class SignupFormComponent {\n  form = new FormGroup({\n    email: new FormControl(\'\', {\n      validators: [Validators.required, Validators.email],\n      asyncValidators: [uniqueEmailValidator(this.userService)]\n    })\n  });\n}\n```\n\n**Status Indicators:**\n```typescript\n// In template\n@if (form.get(\'email\')?.pending) {\n  <span>Checking email availability...</span>\n}\n@if (form.get(\'email\')?.errors?.[\'emailTaken\']) {\n  <span>Email is already taken</span>\n}\n```\n\n**Key Takeaways:**\n1. Async validators run after sync validators pass\n2. Use debounceTime to avoid excessive API calls\n3. Check .pending state for loading indicators\n4. Return null for valid, error object for invalid' }
    ]
  },
  {
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'decorators', title: 'TypeScript Decorators', content: '**Practice - Custom Decorator:**\n```typescript\nfunction Log() {\n  return function (constructor: Function) {\n    console.log(`Created: ${constructor.name}`);\n  };\n}\n\n@Log()\nexport class MyComponent {}\n```\n\n**Key Takeaways:**\n1. Decorators modify class or method behavior\n2. Angular uses decorators extensively (@Component, @Injectable)\n3. Custom decorators can add cross-cutting concerns' },
      { id: 'rxjs-interop', title: 'RxJS Interop (toSignal/toObservable)', content: '**Bridging RxJS and Signals:**\n\n`@angular/core/rxjs-interop` provides utilities to convert between Observables and Signals.\n\n```typescript\nimport { toSignal, toObservable, outputFromObservable } from \'@angular/core/rxjs-interop\';\n\n@Component({})\nexport class UserComponent {\n  private userService = inject(UserService);\n  \n  // Convert Observable to Signal\n  users = toSignal(this.userService.getUsers(), { initialValue: [] });\n  \n  // Convert Signal to Observable\n  name = signal(\'John\');\n  name$ = toObservable(this.name);\n  \n  constructor() {\n    this.name$.subscribe(name => console.log(\'Name changed:\', name));\n  }\n}\n```\n\n**outputFromObservable - Observable as Output:**\n```typescript\n@Component({})\nexport class SearchComponent {\n  search$ = new Subject<string>();\n  \n  // Convert Observable to component output\n  searchOutput = outputFromObservable(this.search$.pipe(\n    debounceTime(300),\n    distinctUntilChanged()\n  ));\n}\n```\n\n**Use Cases:**\n- toSignal: Use Observable data in templates with signals\n- toObservable: Watch signal changes with RxJS operators\n- outputFromObservable: Emit Observable values as component outputs\n\n**Key Takeaways:**\n1. toSignal auto-unsubscribes on component destroy\n2. toObservable converts signal changes to Observable stream\n3. outputFromObservable bridges RxJS to component outputs' }
    ]
  }
];
